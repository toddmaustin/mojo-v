...
encrypted_bootstrap_variability(const uint64e_t encrypted_data[DATASET_SIZE])
{
  ...
  for (uint64_t s = 0; s < SAMPLE_SIZE; ++s)
  {
    ...
    uint64e_t value = encrypted_data[idx];
    sum = sum + value;
    sq_sum = sq_sum + (value * value);
  }

  uint64e_t mean = sum / SAMPLE_SIZE;
...
main(void)
{
  ...
  uint64e_t encrypted_data[DATASET_SIZE];
  for (uint64_t i = 0; i < DATASET_SIZE; ++i)
    encrypted_data[i] = uint64e_t(plain_data[i]);
  ...
  for (uint64_t b = 0; b < BOOTSTRAP_COUNT; ++b)
  {
    uint64_t revealed = results[b].variability.decrypt();
