...
int64e_t
kadane(int64e_t arr[], int n, int64e_t &ends_at)
{
  // stores the maximum sum subarray found so far
  int64e_t max_so_far = 0;
  ends_at = -1;

  // stores the maximum sum of subarray ending at the current position
  int64e_t max_ending_here = 0;
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    // an empty subarray)
    int64e_t _pred = (max_ending_here > 0);
    max_ending_here = cmov(_pred, max_ending_here, 0);

    // update the result if the current subarray sum is found to be greater
    int64e_t _pred1 = (max_so_far < max_ending_here);
    ends_at = cmov(_pred1, i, ends_at);
    max_so_far = cmov(_pred1, max_ending_here, max_so_far);
  }
  return max_so_far;
}
...
main(void)
{
  ...
  // initilize encrypted input data
  for (unsigned i=0; i<SIZE; i++)
    arr[i] = _arr[i];

  int n = SIZE;
  int64e_t max_sum, ends_at;

  libmin_printf("INFO: Array size= %d\n", n);
  {
    // Stopwatch s("VIP_Bench Runtime");
    max_sum = kadane(arr, n, ends_at);
  }
  libmin_printf("INFP: The max sum of a contiguous subarray is %d (ending at index %d)\n", max_sum.decrypt(), ends_at.decrypt());
