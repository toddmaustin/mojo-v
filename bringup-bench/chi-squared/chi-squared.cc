...
encrypted_abs_diff(uint64e_t lhs, uint64e_t rhs)
{
  return cmov(lhs >= rhs, lhs - rhs, rhs - lhs);
}
...
main(void)
{
  ...
  uint64e_t assoc_x[DATASET_SIZE];
  uint64e_t assoc_y[DATASET_SIZE];
  uint64e_t gof_values[DATASET_SIZE];
  ...
  uint64e_t contingency[ASSOC_X_CATEGORIES][ASSOC_Y_CATEGORIES] = {0};
  uint64e_t row_totals[ASSOC_X_CATEGORIES] = {0};
  uint64e_t col_totals[ASSOC_Y_CATEGORIES] = {0};
  uint64e_t gof_observed[GOF_CATEGORIES] = {0};

  for (uint64_t i = 0; i < DATASET_SIZE; ++i)
  {
    for (uint64_t xr = 0; xr < ASSOC_X_CATEGORIES; ++xr)
    {
      uint64e_t x_match = (assoc_x[i] == xr);
      row_totals[xr] += x_match;
      for (uint64_t yc = 0; yc < ASSOC_Y_CATEGORIES; ++yc)
      {
        uint64e_t y_match = (assoc_y[i] == yc);
  ...
  uint64e_t association_chi_scaled = 0;
  for (uint64_t xr = 0; xr < ASSOC_X_CATEGORIES; ++xr)
  {
    for (uint64_t yc = 0; yc < ASSOC_Y_CATEGORIES; ++yc)
    {
      uint64e_t observed_times_n = contingency[xr][yc] * DATASET_SIZE;
      uint64e_t row_col_product = row_totals[xr] * col_totals[yc];
      uint64e_t abs_delta = encrypted_abs_diff(observed_times_n, row_col_product);
      uint64e_t numerator = abs_delta * abs_delta * 1000;
      uint64e_t denominator = (row_col_product * DATASET_SIZE) + (row_col_product == 0);
      association_chi_scaled += numerator / denominator;
    }
  }

  uint64e_t gof_chi_scaled = 0;
  for (uint64_t g = 0; g < GOF_CATEGORIES; ++g)
  {
    uint64e_t expected = gof_expected_counts[g];
    uint64e_t abs_delta = encrypted_abs_diff(gof_observed[g], expected);
    gof_chi_scaled += (abs_delta * abs_delta * 1000) / expected;
  }

  uint64e_t assoc_reject_null = (association_chi_scaled >= 12592);
  uint64e_t gof_reject_null = (gof_chi_scaled >= 9488);
  ...
  for (uint64_t xr = 0; xr < ASSOC_X_CATEGORIES; ++xr)
  {
    row_totals_plain[xr] = row_totals[xr].decrypt();
    for (uint64_t yc = 0; yc < ASSOC_Y_CATEGORIES; ++yc)
      contingency_plain[xr][yc] = contingency[xr][yc].decrypt();
  }

  for (uint64_t yc = 0; yc < ASSOC_Y_CATEGORIES; ++yc)
    col_totals_plain[yc] = col_totals[yc].decrypt();

  for (uint64_t g = 0; g < GOF_CATEGORIES; ++g)
    gof_observed_plain[g] = gof_observed[g].decrypt();

  uint64_t association_chi_plain = association_chi_scaled.decrypt();
  uint64_t gof_chi_plain = gof_chi_scaled.decrypt();
  uint64_t assoc_reject_plain = assoc_reject_null.decrypt();
  uint64_t gof_reject_plain = gof_reject_null.decrypt();
  ...
  libmin_printf("chi-squared: encrypted categorical analysis completed for %u records\n", DATASET_SIZE);
