...
static inline void
exchange(int *a, int i, int j, uint64e_t *swaps)
{
  int temp = a[i];
  a[i] = a[j];
  a[j] = temp;
...
static inline void
compareExchange(int64e_t *a, int i, int j, uint64e_t *swaps)
{
  int64e_t pred = (((int64e_t)(i < j) && (a[i] > a[j]))
                   || ((int64e_t)(i > j) && (a[i] < a[j])));
  int64e_t temp = a[i];
...
static void
permuteRandom(int *a, int size, uint64e_t *swaps)
{
...
static void
compareRegions(int64e_t *a, int s, int t, int offset, uint64e_t *swaps)
{
  int mate[offset];
...
static void
randshellsort(int64e_t *data, uint64_t size, uint64e_t *swaps)
{
  int n = (int)size;
...
main(void)
{
  ...
  int64e_t secret_data[DATASET_SIZE];
  uint64e_t swaps = 0;
