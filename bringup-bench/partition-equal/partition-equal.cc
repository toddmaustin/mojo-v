...
typedef struct {
  uint64e_t can_partition;
  uint64e_t subset_mask;
} partition_result_t;
...
encrypted_partition_equal(const partition_set_t *set)
{
  uint64e_t sum = 0;
  for (uint64_t i = 0; i < set->len; ++i)
    sum = sum + uint64e_t(set->values[i]);

  uint64e_t target = sum / 2;
  uint64e_t odd_sum = sum & 1;

  uint64e_t reachable[MAX_ELEMS + 1][MAX_SUM + 1];
  for (uint64_t i = 0; i <= set->len; ++i)
  {
    for (uint64_t s = 0; s <= MAX_SUM; ++s)
      reachable[i][s] = 0;
  ...
  for (uint64_t i = 0; i < set->len; ++i)
  {
    uint64e_t value = uint64e_t(set->values[i]);

    for (uint64_t s = 0; s <= MAX_SUM; ++s)
    {
      uint64e_t not_take = reachable[i][s];
      uint64e_t take = 0;

      for (uint64_t prev = 0; prev <= MAX_SUM; ++prev)
      {
        uint64e_t transition = (uint64e_t(prev) + value) == uint64e_t(s);
  ...
  uint64e_t reachable_target = 0;
  for (uint64_t s = 0; s <= MAX_SUM; ++s)
  {
    uint64e_t is_target = uint64e_t(s) == target;
    reachable_target = reachable_target || (reachable[set->len][s] && is_target);
  }

  uint64e_t can_partition = (odd_sum == 0) && reachable_target;

  uint64e_t current_sum = target;
  uint64e_t subset_mask = 0;

  for (int64_t i = (int64_t)set->len - 1; i >= 0; --i)
  {
    uint64e_t value = uint64e_t(set->values[i]);
    uint64e_t found = 0;
    uint64e_t prev_sum_choice = current_sum;

    for (uint64_t prev = 0; prev <= MAX_SUM; ++prev)
    {
      uint64e_t matches = reachable[i][prev] && ((uint64e_t(prev) + value) == current_sum);
      uint64e_t choose_here = (!found) && matches;
      prev_sum_choice = cmov(choose_here, uint64e_t(prev), prev_sum_choice);
      found = found || matches;
    }
...
main(void)
{
  ...
  for (uint64_t i = 0; i < N_SETS; ++i)
  {
    uint64_t revealed = secret_results[i].can_partition.decrypt();
    uint64_t revealed_mask = secret_results[i].subset_mask.decrypt();
