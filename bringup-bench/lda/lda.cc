...
typedef uint64e_t VIP_ENCINT;

static const int sizet = 800;
static const int sizec1 = 400;
static const int sizec2 = 400;
...
main(void)
{
...

  {
    ...
    for (int i = 0; i < sizec1; i++)
    {
      ...
      c1pos = cmov(cond, c1pos + (VIP_ENCINT)1, c1pos);
    }
    ...
    for (int i = 0; i < sizec2; i++)
    {
      ...
      c2pos = cmov(cond, c2pos + (VIP_ENCINT)1, c2pos);
    }

    path = cmov(c1pos < c2pos, (VIP_ENCINT)2, (VIP_ENCINT)1);
  }
  ...
  for (int i = 0; i < sizec1; i++)
  {
    ...
    classPrediction[i] = cmov(cond, (VIP_ENCINT)1, (VIP_ENCINT)2);
  }
  ...
  for (int i = 0; i < sizec2; i++)
  {
    ...
    VIP_ENCBOOL cond = (score >= 0.0 && path == (VIP_ENCINT)1) ||
                       (score < 0.0 && path == (VIP_ENCINT)2);
    classPrediction[idx] = cmov(cond, (VIP_ENCINT)1, (VIP_ENCINT)2);
  }

  int c1ok = 0;
  int c2ok = 0;
  for (int i = 0; i < sizec1; i++)
    if (classPrediction[i].decrypt() == 1)
      c1ok++;
  for (int i = 0; i < sizec2; i++)
    if (classPrediction[i + sizec1 - 1].decrypt() == 2)
      c2ok++;

  libmin_printf("LDA results: path=%llu c1=%d/%d c2=%d/%d b0=%lf b1=%lf\n",
                (unsigned long long)path.decrypt(), c1ok, sizec1, c2ok, sizec2,
                b0.decrypt(), b1.decrypt());
